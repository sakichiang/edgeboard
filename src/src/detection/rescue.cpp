    ...
    bool process(Tracking& track, vector<PredictResult> predict) {
        ...
        case Step::Enable: //[02] 使能
        {
            ...
            if (entryLeft)        // 左入库
            {
                ...
                if (_pointNearCone.x >
                    /* 调参点 */ ROWSIMAGE * 0.32) // 当车辆开始靠近右边锥桶：准备入库
                {
                    counterRec++;
                    if (counterRec >= 2) {
            ...
            else // 右入库
            {
                printf("right\n");
                _pointNearCone = getConeLeftDown(track.pointsEdgeLeft,
                    pointConeLeft); // 搜索左下锥桶(原右下)
                if (_pointNearCone.x >
                    ROWSIMAGE * 0.32) // 当车辆开始靠近右边锥桶：准备入库
                {
                    counterRec++;
                    if (counterRec >= 2) {
                        step = Step::Enter; // 进站使能
        ...
        case Step::Enter: //[03] 入库使能
        {
            ...
            if (counterSession > 12) {
                if (track.pointsEdgeLeft.size() > ROWSIMAGE / 2 &&
                    track.pointsEdgeRight.size() > ROWSIMAGE / 2) {
                    counterExit++;
					printf("counterExit is %d ", counterExit);
                    /* 调参点 */ if (counterExit > 25) {
						printf("counterExit is inininininini\n ");
                        counterExit = 0;
                        step = Step::Stop; // 停车使能
                        counterRec = 0;
                        counterSession = 0;
                ...
                {
					printf("counterRec is %d",counterRec);
                    counterRec++;
                    if (counterRec > 1) {
                        counterRec = 0;
        ...
        {
            bydcounter++;
            printf("yse yes yes yes yes yes yes yes yes yes \n");
            counterSession++;
            if (counterSession > 10) // 超时保护
            {
				printf("stop cruise session\n");
                step = Step::Stop; // 停车使能
                counterRec = 0;
                counterSession = 0;
                counterExit = 0;
            ...
            if (track.pointsEdgeLeft.size() > ROWSIMAGE / 2 &&
                track.pointsEdgeRight.size() > ROWSIMAGE / 2) {
                printf("left and right bigger than half");
                counterExit++;
                if (counterExit > 13) {
                    printf("stop cruise exit\n");
                    counterExit = 0;
                    step = Step::Stop; // 停车使能
                    counterRec = 0;
                }
            ...
            if (heighestCone.x > 0 && heighestCone.y > 0) {
                ...
                if (pointConeLeft.size() + pointConeRight.size() > 0)
                    ...
                    levelCones = 0;
                if (levelCones > ROWSIMAGE * 0.3 || levelCones == 0) {//修改点0.5
                    printf("lever counterRec is %d\n", counterRec);
                    counterRec++;
                    if (counterRec > 1) {//修改点
                        printf("counterRec stop\n");
                        step = Step::Stop; // 停车使能
                        counterRec = 0;
                    }
                }
