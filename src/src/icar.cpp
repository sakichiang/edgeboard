...
int main(int argc, char const *argv[]) {
  ...
  while (1) {
	...
	// // //[10] 坡道区检测
    if ((scene == Scene::BridgeScene || scene == Scene::NormalScene)&& 
    ...
    // Mat ii=imgCorrect.clone();
    //      ctrlCenter.drawImage(tracking,startTime
    //                         ii); 
    // savePicture(ii);
  //elements(tracking);
    //[12] 车辆控制中心拟合
    ...
    motion.poseCtrl1(ctrlCenter.controlCenter, startTime-preTime); // 姿态控制（舵机 附带时间
    // mut.lock();
    // start_uart=true;
    // mut.unlock();
	printf("速度: %0.4f\n", speed);
