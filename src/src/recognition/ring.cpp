...
{
public:
    int counterShield = 114514-60; // 环岛检测屏蔽计数器：屏蔽车库误检测
    ...
    void reset(void)
    {
        ...
        count=0;
		errorCount = 0;
		counterShield = 0;  // 确保在环岛检测正确被触发后不会再次触发
        k=0;
        finish=0;
    }
    /**
    ...
    */
    ...
    void reset1(void)
    {
        ...
        count = 0;
        errorCount = 0;
		counterShield = 114514 - 60;  // 确保在环岛检测被错误触发后的一段时间内不会再次触发
        k = 0;
        finish = 0;
    }
    /**
    ...
    {
        if (counterShield < 114514)// 确保在环岛检测被触发后的一段时间内不会再次触发
        {
            counterShield++;
			printf("环岛屏蔽中%d\n",counterShield);
            return false;
        ...
        //如果进入状态时间过长则为错误圆环停止判环
		if (ringStep == RingStep::Entering || ringStep == RingStep::Inside || ringStep == RingStep::Circle) {
			errorCount++;
			if (errorCount > 180) {
                printf("错误进入圆环强制退出\n");
				ringStep = RingStep::None;
